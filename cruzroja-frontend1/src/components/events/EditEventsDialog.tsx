...
export function EditEventDialog({
        ...
        skill_quota:
            Array.isArray(initial.skill_quota) && initial.skill_quota.length
                ? initial.skill_quota.map((sq) => ({
                  id: String(sq.id),
                  name: sq.name,
                  quantity: Number.isFinite(Number(sq.quantity))
                      ? Number(sq.quantity)
                      : 0,
                }))
                : [],
  ...
  const resolvedLeaderName = useMemo(() => {
    ...
    const match = users.find(
        (u) => String((u as any).document ?? (u as any).id) === form.attendant,
    );
    return match ? (match as any).name ?? "" : "";
  }, [leaderName, users, form.attendant]);

  function onChange<K extends keyof EditEventValues>(
      k: K,
      v: EditEventValues[K],
  ) {
    setForm((f) => ({ ...f, [k]: v }));
  ...
  function addQuotaRow() {
    setForm((f) => ({
      ...f,
      skill_quota: [...f.skill_quota, { id: "", quantity: 0 }],
    }));
  }
  function removeQuotaRow(index: number) {
    setForm((f) => ({
      ...f,
      skill_quota: f.skill_quota.filter((_, i) => i !== index),
    }));
  }
    ...
    try {
      ...
      await toast.promise(op, {
        ...
        success: (res: { message?: string }) => (
            <b>{res.message ?? "Evento actualizado"}</b>
        ),
        error: (res: { message?: string }) => (
            <b>{res.message ?? "No se pudo actualizar"}</b>
        ),
      });
