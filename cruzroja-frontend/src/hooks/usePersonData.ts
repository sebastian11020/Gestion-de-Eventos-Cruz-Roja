...
import {getReportDesvinculate, getReportInactivate} from "@/services/serviceSelect";

function normalizeGroups(input: unknown): groupReport["groups"] {
    if (!input) return [];
    if (Array.isArray(input) && input.every(it => it && Array.isArray((it as any).volunteers))) {
...
export function usePersonData() {
    ...
    async function loadAll() {
        setLoading(true);
        try {
            const [usersData, groupsData,inactivateData] = await Promise.all([getPerson(), getReportDesvinculate(),getReportInactivate()]);
            setUsers(usersData ?? []);
