...
export default function CreateEventForm({
  ...
  const sectionalSelected = useMemo(
      () =>
          sectionals.find(
              (c) => String(c.id) === String(form.sectionalId),
          ) || null,
  ...
  async function handleSubmit(
    ...
    try {
      const cap = Number.isFinite(form.capacity)
          ? form.capacity
          : 0;
      ...
      await toast.promise(
          ...
          {
            loading:
                "Creando evento, esto puede tardar unos instantes",
