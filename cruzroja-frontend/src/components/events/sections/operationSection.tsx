...
export function OperationSection({
    ...
    const setQty = (id: string, nextRaw: string) => {
        const parsed = nextRaw === "" ? 0 : Math.max(0, Math.floor(Number(nextRaw) || 0));
        const current = getQty(id);
        const assignedWithoutThis =
            quotas.reduce((sum, q) => (q.id === id ? sum : sum + (Number.isFinite(q.qty) ? q.qty : 0)), 0);
        ...
        <FancyCard title="Operación" icon={<ListChecks className="w-4 h-4" />}>
            ...
            <div className="grid gap-4 sm:grid-cols-4">
                <Field label="Capacidad">
                    <input
                        ...
                        onBlur={() =>
                            onChange("capacity", Number.isFinite(form.capacity) ? Math.max(0, form.capacity) : 0)
                        }
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2">
                ...
                <div className="text-sm text-right">
                    <span className="font-medium">Asignado:</span> {totalAssigned} / {capacity}{" "}
                    {overCapacity && <span className="ml-2 text-red-600">Se excede la capacidad</span>}
                </div>
            </div>
            ...
            {requiresSkills && (
                <div className="mt-3 rounded-xl border border-gray-200 p-3">
                    <div className="grid gap-3">
                        {skills.length === 0 && (
                            <div className="text-sm text-gray-500">No hay habilidades configuradas.</div>
                        )}
                        ...
                        {skills.map((sk) => {
                            ...
                            return (
                                <div key={sk.id} className="flex items-center justify-between gap-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        />
                                        <span className="text-xs text-gray-500">máx. {maxAllowed}</span>
                                    </div>
                                </div>
                            );
                        })}
                        ...
                        <div className="flex items-center justify-between gap-4">
                            <div className="text-sm font-medium">Base (voluntarios sin habilidad)</div>
                            <div className="flex items-center gap-2">
                                <input
                                    type="number"
                                    readOnly
