...
export function Topbar({ onOpenSidebar }: Props) {
  ...
  const [notifications, setNotifications] = useState<string[]>([
    ...
    "Gracias por participar en el evento estadio",
  ]);
  ...
  return (
    <header className="sticky top-0 z-40 w-full bg-white shadow-lg backdrop-blur">
      <div className="mx-auto flex h-12 md:h-14 items-center px-3 sm:px-6 lg:px-8 gap-3">
        {/* Botón móvil: Cruz Roja + Dashboard */}
        <button
          type="button"
                    ...
                    {count === 0 ? (
                      ...
                      <ul className="divide-y divide-slate-100">
                        {notifications.map((n, i) => (
                          <li key={i}>
                            <button
                              className="group w-full text-left px-4 py-3 hover:bg-slate-50 focus:bg-slate-50 transition"
                              onClick={() => {
                                setOpen(false);
                              }}
                            >
