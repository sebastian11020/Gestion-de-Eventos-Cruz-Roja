...
import {
    Home, Layers, Clipboard, Users, CalendarCheck, Settings, LogOut, ChartColumnBig, Warehouse,
} from "lucide-react";
...
export function AppSidebar({
    ...
    const [role, setRole] = useState<Role | undefined>(() => normalizeRole(initialRole));
                ...
                {makeInitials(user?.name, user?.lastName)}
                            ...
                            <div className="min-w-0">
                                <p className="truncate text-[15px] font-bold leading-tight">
                                    {[firstToken(user?.name), firstToken(user?.lastName)].filter(Boolean).join(" ") || "Usuario"}
                                </p>
