// components/layout/volunteerPickerModal.tsx
"use client";
...
type Volunteer = { id: string; name: string; document?: string; email?: string };
...
export default function VolunteerPickerModal({
    ...
    const all: Volunteer[] = useMemo(
        () => [
            { id: "v1", name: "Ana Rodríguez", document: "1234567890", email: "ana@x.com" },
            { id: "v2", name: "Carlos Pérez", document: "1098765432", email: "carlos@x.com" },
            { id: "v3", name: "Luisa Gómez", document: "555555555", email: "luisa@x.com" },
            { id: "v4", name: "Mario López", document: "888888888", email: "mario@x.com" },
        ],
        []
    );
                ...
                Mostrando {filtered.length === 0 ? 0 : (page - 1) * PAGE + 1}–
                  ...
                  {selected.length}
                        ...
                        <div className="max-h-[360px] overflow-auto p-3 space-y-2">
                            {selected.length === 0 && (
                                <div className="text-sm text-gray-600">Aún no has agregado voluntarios.</div>
                            )}
                            ...
                            {selected.map((u) => (
                                ...
                                >
                                    <div className="truncate">
                                        ...
                                        <div className="text-xs text-gray-500 truncate">{u.document ?? "-"}</div>
                                    </div>
                                    <button
                                        type="button"
                                        onClick={() => setSelected((prev) => prev.filter((v) => v.id !== u.id))}
                        ...
                        <div className="sticky bottom-0 flex items-center justify-end gap-2 rounded-b-3xl bg-gradient-to-r from-blue-50 to-indigo-50 p-3">
                            <Button variant="outline" className="rounded-xl" type="button" onClick={onClose}>
                                Cancelar
                            </Button>
                            <Button
