...
export function InactiveTable({
    ...
    const allPageSelected = pageSlice.length > 0 && pageSlice.every((it) => selection.isSelected(it));

    return (
        <section className="space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3">
                <h2 className="text-sm font-semibold text-gray-700">Listado para resolución de desvinculación</h2>
                <PaginationBar meta={{ showingFrom: pagination.showingFrom, showingTo: pagination.showingTo, total }}
                    ...
                    <table className="min-w-full text-left text-sm">
                        ...
                        {pageSlice.length === 0 ? (
                            ...
                            pageSlice.map((it) => {
                                ...
                                return (
                                    <tr key={it.document} className={`border-t border-gray-100 hover:bg-gray-50/60 ${checked ? "bg-red-50/50" : ""}`}>
                                        ...
                                        <td className="sticky left-12 z-10 bg-white px-4 py-2 font-medium text-gray-800">{it.document}</td>
                                        <td className="px-4 py-2 text-gray-700">{it.license}</td>
                                        <td className="px-4 py-2 text-gray-800">{it.name}</td>
                                        <td className="px-4 py-2 text-gray-800">{fmtDate(it.Inactivation_date)}</td>
                                        <td className="px-4 py-2">
                                            <input
                                                type="text"
                                                value={notes[it.document] ?? ""}
                                                onChange={(e) => setNotes((prev) => ({ ...prev, [it.document]: e.target.value }))}
