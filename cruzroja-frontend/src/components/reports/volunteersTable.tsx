...
export function VolunteersTable({
    ...
    setNotes: (f: (prev: Record<string, string>) => Record<string, string>) => void;
    onOpenHours: (v: Volunteer) => void;
    onGeneratePdf: () => Promise<void> | void;
}) {
    const allPageSelected = pageSlice.length > 0 && pageSlice.every((v) => selection.isSelected(v));

    return (
        <section className="space-y-4">
            <h1 className="text-xl font-bold tracking-tight text-gray-800 md:text-2xl">Reportes</h1>

            <div className="flex flex-wrap items-center justify-between gap-3">
                <div className="text-sm">
                    Seleccionados: <span className="font-semibold">{selection.count()}</span>
                </div>
            ...
            <div className="overflow-hidden rounded-xl bg-white ring-1 ring-gray-200 shadow-sm">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 border-b border-gray-200 px-4 py-3">
                    <h2 className="text-sm font-semibold text-gray-700">Listado de voluntarios</h2>
                    <PaginationBar meta={{
                ...
                <div className="overflow-auto">
                    <table className="min-w-full text-left text-sm">
                        ...
                        <tr>
                            ...
                            <th className="sticky left-12 z-10 bg-gray-50 px-4 py-2 font-medium">Documento</th>
                            <th className="px-4 py-2 font-medium">Carnet</th>
                            <th className="px-4 py-2 font-medium">Nombre</th>
                            <th className="px-4 py-2 font-medium w-[360px]">Observaciones</th>
                        ...
                        {loading ? (
                            <tr>
                                <td colSpan={6} className="px-4 py-10 text-center text-sm text-gray-500">Cargando…</td>
                            </tr>
                        ) : pageSlice.length === 0 ? (
                            <tr>
                                <td colSpan={6} className="px-4 py-10 text-center text-sm text-gray-500">No hay datos para mostrar.</td>
                            </tr>
                        ) : (
                            pageSlice.map((v) => {
                                const checked = selection.isSelected(v);
                                return (
                                    <tr key={v.document} className={`border-t border-gray-100 hover:bg-gray-50/60 ${checked ? "bg-red-50/50" : ""}`}>
                                        ...
                                        <td className="sticky left-12 z-10 bg-white px-4 py-2 font-medium text-gray-800">{v.document}</td>
                                        <td className="px-4 py-2 text-gray-700">{(v as any).licence ?? (v as any).license ?? ""}</td>
                                        <td className="px-4 py-2 text-gray-800">{v.name}</td>
                                        <td className="px-4 py-2">
                                            <input
                                                type="text"
                                                value={notes[v.document] ?? ""}
                                                onChange={(e) => setNotes((prev) => ({ ...prev, [v.document]: e.target.value }))}
                ...
                <div className="border-t border-gray-200 px-4 py-3">
                    <PaginationBar
                        meta={{ showingFrom: pagination.showingFrom, showingTo: pagination.showingTo, total }}
                        page={pagination.page}
                        totalPages={pagination.totalPages}
                        onPage={pagination.goTo}
                        pageSize={pagination.pageSize}
